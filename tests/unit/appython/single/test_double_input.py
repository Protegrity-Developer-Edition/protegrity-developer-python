...
def test():
    with start_protect_unprotect_mocking():
        ...
        input_data = 12345567.1
        
        protected = session.protect(input_data, "number")
        un_protected = session.unprotect(protected, "number")
        # reprotected = session.reprotect(protected, "number","SUCCESS_REPROTECT_DOUBLE")
        # re_unprotected = session.unprotect(reprotected, "SUCCESS_REPROTECT_DOUBLE")
...
def test_exiv():
    with start_protect_unprotect_mocking():
        ...
        # reprotected = session.reprotect(protected, "number","SUCCESS_REPROTECT_DOUBLE",old_external_iv=bytes("oldexiv","utf-8"),new_external_iv=bytes("newexiv","utf-8"))
        # re_unprotected = session.unprotect(reprotected, "SUCCESS_REPROTECT_DOUBLE",external_iv=bytes("newexiv","utf-8"))
...
def test_re_invalid_exiv():
    with start_protect_unprotect_mocking():
        ...
        try:
            session.reprotect(input_data, "number","SUCCESS_REPROTECT_DOUBLE",old_external_iv="inv",new_external_iv="inv")
            assert False, "Expected an exception but none was raised"
        except Exception as e:
            assert str(e) == "-1, Invalid Keyword Type for keyword: old_external_iv!! Expected: bytes, Actual: <class 'str'>"
        try:
            session.reprotect(input_data, "number","SUCCESS_REPROTECT_DOUBLE",old_external_iv=bytes("exiv","utf-8"),new_external_iv="inv")
            assert False, "Expected an exception but none was raised"
        except Exception as e:
            assert str(e) == "-1, Invalid Keyword Type for keyword: new_external_iv!! Expected: bytes, Actual: <class 'str'>"
        try:
            session.reprotect(input_data, "number","SUCCESS_REPROTECT_DOUBLE",old_external_iv=bytes("exiv","utf-8"))
            assert False, "Expected an exception but none was raised"
        except Exception as e:
...
def test_invalid_charset():
    with start_protect_unprotect_mocking():
        ...
        try:
            ...
            assert str(e) == "-1, Charset is only supported with byte input data type"
        try:
            session.reprotect(input_data, "number","SUCCESS_REPROTECT_DOUBLE",charset=Charset.UTF8)
            assert False, "Expected an exception but none was raised"
        except Exception as e:
            assert str(e) == "-1, Charset is only supported with byte input data type"
...
def test_invalid_username():
    with start_protect_unprotect_mocking():
        ...
        try:
            ...
            assert str(e) == "3, The user does not have the appropriate permissions to perform the requested operation."
        # try:
        #     session.reprotect(input_data, "number","SUCCESS_REPROTECT_DOUBLE")
        # except Exception as e:
        #     assert str(e) == "3, The user does not have the appropriate permissions to perform the requested operation."
...
def test_invalid_data_element():
    with start_protect_unprotect_mocking():
        ...
        try:
            ...
            assert str(e) == "2, The data element could not be found in the policy."
        # try:
        #     session.reprotect(input_data, "INVALID_DE","SUCCESS_REPROTECT_DOUBLE")
        # except Exception as e:
        #     assert str(e) == "2, The data element could not be found in the policy."
        # try:
