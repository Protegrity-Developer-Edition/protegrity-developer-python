...
def test_str():
    with start_protect_unprotect_mocking():
        ...
        un_protected = session.unprotect(protected, "zipcode")
        # reprotected = session.reprotect(protected, "zipcode","SUCCESS_REPROTECT_STR")
...
def test_str_exiv():
    with start_protect_unprotect_mocking():
        ...
        un_protected = session.unprotect(protected, "zipcode",external_iv=bytes("exiv","utf-8"))
        # reprotected = session.reprotect(protected, "zipcode","SUCCESS_REPROTECT_STR",old_external_iv=bytes("oldexiv","utf-8"),new_external_iv=bytes("newexiv","utf-8"))
...
def test_str_invalid_username():
    with start_protect_unprotect_mocking():
        ...
        try:
            ...
            assert str(e) == "3, The user does not have the appropriate permissions to perform the requested operation."
        # try:
        #     session.reprotect(input_data, "zipcode","SUCCESS_REPROTECT_STR")
        # except Exception as e:
        #     assert str(e) == "3, The user does not have the appropriate permissions to perform the requested operation."
...
def test_str_invalid_data_element():
    with start_protect_unprotect_mocking():
        ...
        #     assert str(e) == "2, The data element could not be found in the policy."
        # try:
        #     session.reprotect(input_data, "zipcode","INVALID_DE")
        # except Exception as e:
        #     assert str(e) == "2, The data element could not be found in the policy."
