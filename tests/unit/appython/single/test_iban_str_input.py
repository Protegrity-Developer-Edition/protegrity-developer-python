...
def test_str():
    with start_protect_unprotect_mocking():
        ...
        input_data = "QA54QNBA000000000000693123456"
        
        protected = session.protect(input_data, "iban")
        un_protected = session.unprotect(protected, "iban")
        # reprotected = session.reprotect(protected, "iban","SUCCESS_REPROTECT_STR")
...
@pytest.mark.skip(reason="Exiv Test Unsupported")
def test_str_exiv():
    with start_protect_unprotect_mocking():
        protector = Protector()
        session = protector.create_session("superuser")
        ...
        protected = session.protect(input_data, "iban",external_iv=bytes("exiv","utf-8"))
        un_protected = session.unprotect(protected, "iban",external_iv=bytes("exiv","utf-8"))
        # reprotected = session.reprotect(protected, "iban","SUCCESS_REPROTECT_STR",old_external_iv=bytes("oldexiv","utf-8"),new_external_iv=bytes("newexiv","utf-8"))
...
def test_str_invalid_username():
    with start_protect_unprotect_mocking():
        ...
        try:
            ...
            assert str(e) == "3, The user does not have the appropriate permissions to perform the requested operation."
        # try:
        #     session.reprotect(input_data, "iban","SUCCESS_REPROTECT_STR")
        # except Exception as e:
        #     assert str(e) == "3, The user does not have the appropriate permissions to perform the requested operation."
...
def test_str_invalid_data_element():
    with start_protect_unprotect_mocking():
        ...
        #     assert str(e) == "2, The data element could not be found in the policy."
        # try:
        #     session.reprotect(input_data, "iban","INVALID_DE")
        # except Exception as e:
        #     assert str(e) == "2, The data element could not be found in the policy."
