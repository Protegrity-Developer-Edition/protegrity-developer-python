...
def test_str():
    with start_protect_unprotect_mocking():
        ...
        input_data = "RBR132654879"
        
        protected = session.protect(input_data, "passport")
        un_protected = session.unprotect(protected, "passport")
        # reprotected = session.reprotect(protected, "passport","SUCCESS_REPROTECT_STR")
...
#extiv test is commented as exiv is not supported for passport data element
def test_str_exiv():
    with start_protect_unprotect_mocking():
        protector = Protector()
        session = protector.create_session("superuser")
        ...
        protected = session.protect(input_data, "passport",external_iv=bytes("exiv","utf-8"))
        un_protected = session.unprotect(protected, "passport",external_iv=bytes("exiv","utf-8"))
        # reprotected = session.reprotect(protected, "passport","SUCCESS_REPROTECT_STR",old_external_iv=bytes("oldexiv","utf-8"),new_external_iv=bytes("newexiv","utf-8"))
...
def test_str_re_invalid_exiv():
    with start_protect_unprotect_mocking():
        ...
        try:
            session.reprotect(input_data, "passport","SUCCESS_REPROTECT_STR",old_external_iv="inv",new_external_iv="inv")
            assert False, "Expected an exception but none was raised"
        except Exception as e:
            assert str(e) == "-1, Invalid Keyword Type for keyword: old_external_iv!! Expected: bytes, Actual: <class 'str'>"
        try:
            session.reprotect(input_data, "passport","SUCCESS_REPROTECT_STR",old_external_iv=bytes("exiv","utf-8"),new_external_iv="inv")
            assert False, "Expected an exception but none was raised"
        except Exception as e:
            assert str(e) == "-1, Invalid Keyword Type for keyword: new_external_iv!! Expected: bytes, Actual: <class 'str'>"
        try:
            session.reprotect(input_data, "passport","SUCCESS_REPROTECT_STR",old_external_iv=bytes("exiv","utf-8"))
            assert False, "Expected an exception but none was raised"
        except Exception as e:
...
def test_str_invalid_charset():
    with start_protect_unprotect_mocking():
        ...
        try:
            session.protect(input_data, "passport",charset=Charset.UTF8)
            assert False, "Expected an exception but none was raised"
        except Exception as e:
            assert str(e) == "-1, Charset is only supported with byte input data type"
        try:
            session.unprotect(input_data, "passport",charset=Charset.UTF8)
            assert False, "Expected an exception but none was raised"
        except Exception as e:
            assert str(e) == "-1, Charset is only supported with byte input data type"
        try:
            session.reprotect(input_data, "passport","SUCCESS_REPROTECT_STR",charset=Charset.UTF8)
            assert False, "Expected an exception but none was raised"
        except Exception as e:
            assert str(e) == "-1, Charset is only supported with byte input data type"
...
def test_str_invalid_username():
    with start_protect_unprotect_mocking():
        ...
        try:
            session.protect(input_data, "passport")
            assert False, "Expected an exception but none was raised"
        except Exception as e:
            assert str(e) == "3, The user does not have the appropriate permissions to perform the requested operation."
        try:
            session.unprotect(input_data, "passport")
            assert False, "Expected an exception but none was raised"
        except Exception as e:
            assert str(e) == "3, The user does not have the appropriate permissions to perform the requested operation."
        # try:
        #     session.reprotect(input_data, "passport","SUCCESS_REPROTECT_STR")
        # except Exception as e:
        #     assert str(e) == "3, The user does not have the appropriate permissions to perform the requested operation."
...
def test_str_invalid_data_element():
    with start_protect_unprotect_mocking():
        ...
        #     assert str(e) == "2, The data element could not be found in the policy."
        # try:
        #     session.reprotect(input_data, "passport","INVALID_DE")
        # except Exception as e:
        #     assert str(e) == "2, The data element could not be found in the policy."
