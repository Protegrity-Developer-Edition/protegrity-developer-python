...
def test_str():
    with start_protect_unprotect_mocking():
        ...
        input_data = "sdk@protegrity.com"
        
        protected = session.protect(input_data, "email")
        un_protected = session.unprotect(protected, "email")
        # reprotected = session.reprotect(protected, "email","SUCCESS_REPROTECT_STR")
...
def test_str_exiv():
    with start_protect_unprotect_mocking():
        ...
        protected = session.protect(input_data, "email",external_iv=bytes("exiv","utf-8"))
        un_protected = session.unprotect(protected, "email",external_iv=bytes("exiv","utf-8"))
        # reprotected = session.reprotect(protected, "email","SUCCESS_REPROTECT_STR",old_external_iv=bytes("oldexiv","utf-8"),new_external_iv=bytes("newexiv","utf-8"))
...
def test_str_protect_unprotect_invalid_exiv():
    with start_protect_unprotect_mocking():
        ...
        try:
            session.protect(input_data, "email",external_iv="inv")
        except Exception as e:
            assert str(e) == "-1, Invalid Keyword Type for keyword: external_iv!! Expected: bytes, Actual: <class 'str'>"
        try:
            session.unprotect(input_data, "email",external_iv="inv")
        except Exception as e:
            assert str(e) == "-1, Invalid Keyword Type for keyword: external_iv!! Expected: bytes, Actual: <class 'str'>"
...
def test_str_re_invalid_exiv():
    with start_protect_unprotect_mocking():
        ...
        try:
            session.reprotect(input_data, "email","SUCCESS_REPROTECT_STR",old_external_iv="inv",new_external_iv="inv")
        except Exception as e:
            assert str(e) == "-1, Invalid Keyword Type for keyword: old_external_iv!! Expected: bytes, Actual: <class 'str'>"
        try:
            session.reprotect(input_data, "email","SUCCESS_REPROTECT_STR",old_external_iv=bytes("exiv","utf-8"),new_external_iv="inv")
        except Exception as e:
            assert str(e) == "-1, Invalid Keyword Type for keyword: new_external_iv!! Expected: bytes, Actual: <class 'str'>"
        try:
            session.reprotect(input_data, "email","SUCCESS_REPROTECT_STR",old_external_iv=bytes("exiv","utf-8"))
        except Exception as e:
...
def test_str_invalid_charset():
    with start_protect_unprotect_mocking():
        ...
        try:
            session.protect(input_data, "email",charset=Charset.UTF8)
        except Exception as e:
            assert str(e) == "-1, Charset is only supported with byte input data type"
        try:
            session.unprotect(input_data, "email",charset=Charset.UTF8)
        except Exception as e:
            assert str(e) == "-1, Charset is only supported with byte input data type"
        try:
            session.reprotect(input_data, "email","SUCCESS_REPROTECT_STR",charset=Charset.UTF8)
        except Exception as e:
            assert str(e) == "-1, Charset is only supported with byte input data type"
...
def test_str_invalid_username():
    with start_protect_unprotect_mocking():
        ...
        try:
            session.protect(input_data, "email")
        except Exception as e:
            assert str(e) == "3, The user does not have the appropriate permissions to perform the requested operation."
        try:
            session.unprotect(input_data, "email")
        except Exception as e:
            assert str(e) == "3, The user does not have the appropriate permissions to perform the requested operation."
        # try:
        #     session.reprotect(input_data, "email","SUCCESS_REPROTECT_STR")
        # except Exception as e:
        #     assert str(e) == "3, The user does not have the appropriate permissions to perform the requested operation."
...
def test_str_invalid_data_element():
    with start_protect_unprotect_mocking():
        ...
        #     assert str(e) == "2, The data element could not be found in the policy."
        # try:
        #     session.reprotect(input_data, "email","INVALID_DE")
        # except Exception as e:
        #     assert str(e) == "2, The data element could not be found in the policy."
