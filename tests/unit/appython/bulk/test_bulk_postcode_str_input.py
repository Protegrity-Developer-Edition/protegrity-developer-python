...
def test_bulk_str():
    with start_protect_unprotect_mocking():
        ...
        un_protected = session.unprotect(protected[0], "postcode")
...
def test_str_exiv():
    with start_protect_unprotect_mocking():
        protector = Protector()
        session = protector.create_session("superuser")
        input_data = ["SW1A 1AA", "M1 1AA", "E1 6AN"]
    
        protected = session.protect(input_data, "postcode",external_iv=bytes("exiv","utf-8"))
        un_protected = session.unprotect(protected[0], "postcode",external_iv=bytes("exiv","utf-8"))
