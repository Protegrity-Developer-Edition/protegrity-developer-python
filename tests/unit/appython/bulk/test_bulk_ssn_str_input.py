...
def test_bulk_ssn():
    with start_protect_unprotect_mocking():
        ...
        un_protected = session.unprotect(protected[0], "ssn")
...
def test_ssn_exiv():
    with start_protect_unprotect_mocking():
        ...
        un_protected = session.unprotect(protected[0], "ssn",external_iv=bytes("exiv","utf-8"))

        for index in range(len(input_data)):
            assert input_data[index] == un_protected[0][index]

def test_ssn_protect_unprotect_invalid_exiv():
    with start_protect_unprotect_mocking():
        protector = Protector()
        session = protector.create_session("superuser")
        input_data = ["123456789", "987654321", "555443333"]
...
def test_ssn_invalid_data_element():
    with start_protect_unprotect_mocking():
        protector = Protector()
        session = protector.create_session("superuser")
        input_data = ["123456789", "987654321", "555443333"]
