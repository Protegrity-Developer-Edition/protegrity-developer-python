...
def test_bulk_str():
    with start_protect_unprotect_mocking():
        ...
        un_protected = session.unprotect(protected[0], "email")
...
def test_str_exiv():
    with start_protect_unprotect_mocking():
        ...
        un_protected = session.unprotect(protected[0], "email",external_iv=bytes("exiv","utf-8"))
...
def test_str_protect_unprotect_invalid_exiv():
    with start_protect_unprotect_mocking():
        protector = Protector()
        session = protector.create_session("superuser")
        input_data = ["john.doe@example.com", "jane.smith@company.org", "admin@testsite.net"]
