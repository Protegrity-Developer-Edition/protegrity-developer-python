...
def test_bulk_str():
    with start_protect_unprotect_mocking():
        ...
        un_protected = session.unprotect(protected[0], "nin")

        for index in range(len(input_data)):
            assert input_data[index] == un_protected[0][index]

# nin does not support external_iv, so commenting out exiv tests
# def test_str_exiv():
#     protector = Protector()
#     session = protector.create_session("superuser")
#     input_data = ["AE778453K", "BH123456C", "CD789012E"]
    
#     protected = session.protect(input_data, "nin",external_iv=bytes("exiv","utf-8"))
#     un_protected = session.unprotect(protected[0], "nin",external_iv=bytes("exiv","utf-8"))
